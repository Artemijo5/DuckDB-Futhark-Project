...
-- TODO gather + join + entry points
-- I mean, ig I've already kinda done the gather part in the sort func (...)
...
: idx_t.t =
	let loop_over =
		...
		while (!def_lt && !def_gt && b<(i64.min l1 l2)) do
			let cmp = char_cmp str1[b] str2[b]
			let dlt = cmp<0
			let dgt = cmp>0
			in (dlt, dgt, b+1)
	in
...
let str_cmp_in_content [n] [total_len]
	...
	(char_cmp : u8 -> u8 -> i32)
: idx_t.t =
	let (inf1, inf2) = (str_idx[i1], str_idx[i2])
	let (l1, l2) = (get_str_len total_len str_idx i1, get_str_len total_len str_idx i2)
	let loop_over =
		loop (def_lt, def_gt, b) = (false, false, 0)
		while (!def_lt && !def_gt && b<(i64.min l1 l2)) do
			let cmp = char_cmp str_content[inf1+b] str_content[inf2+b]
			let dlt = cmp<0
			let dgt = cmp>0
			in (dlt, dgt, b+1)
	in
...
: ([total_len]u8, [n]idx_t.t, [n]t) =
	...
	let (sorted_str_con, sorted_str_idx) =
		...
		let new_con =
			loop buff = (replicate total_len 0)
