...
-- | Perform bsearch_first implementation using Merge-Path co-partitioning.
--
-- For as, bs sorted,
-- for each value in as,
-- find the index of the first match in bs.
def bsearch_first_merge_path [na] [nb] 't
    (eq : t -> t -> bool)
    (geq: t -> t -> bool)
    (leq: t -> t -> bool)
    (gt : t -> t -> bool)
...
: [ts](i64,i64) =
    ...
    -- Perform binary search
    let match_range_first = a_ranges |> bsearch_last
        (\(ia,iA) (ib,_) -> iA>=ia && ia<na &&
            (ib==0 || (as[ia] `geq` bs[ib]))
        )
        (\(ia,iA) (ib,_) -> ia==na || iA<ia ||
            (as[ia] `lt` bs[ib]))
        (replicate ts 0)
        (replicate ts (length b_ranges))
        b_ranges
