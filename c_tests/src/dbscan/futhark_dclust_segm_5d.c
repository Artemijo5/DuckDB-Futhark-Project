...
int main(int argc, char *argv[]) {
    ...
    // Initializations
    	...
    	char ch;
	    while(
	    	(ch = getopt_long_only(argc,argv,"i:o:s:c:1:2:3:4:5:e:m:I:L:",long_options,NULL)) != -1
	    ) {
	      switch(ch) {
	        case 'i':
	...
	for(int64_t iter_j=0; iter_j<ITER; iter_j++) {
		...
		// Query to read data from input file.
			...
			sprintf(query_str,
				"SELECT (#1)::DOUBLE,(#2)::DOUBLE,(#3)::DOUBLE,(#4)::DOUBLE,(#5)::DOUBLE FROM read_csv('%s') LIMIT %ld;",
