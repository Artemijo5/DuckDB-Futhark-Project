...
#include "../../../ft_clibs/dbscan_entry.h"

#include <unistd.h>
#include <getopt.h>

#define default_INPUT_FILENAME  "2d.txt"
#define default_OUTPUT_FILENAME "futhark_dbscan_2d.csv"
...
int main(int argc, char *argv[]) {
    ...
    // Initializations
    	...
    	char ch;
	    while(
	    	(ch = getopt_long_only(argc,argv,"i:o:s:ge:m:L:",long_options,NULL)) != -1
	    ) {
	      switch(ch) {
	        case 'i':
	...
	// init logger
		...
		FILE* logfile = loginit(LOGFILE, "Starting futhark-DBSCAN test program.");
	    if(LOGFILE && !logfile) {
	...
	// Do dbscan
		...
		if(!USE_GRID) {
			...
			futhark_entry_do_kd_dbscan_2d_f64(ctx, &dbscan_res,
			...
			int64_t subdiv = (int64_t)ceil(sqrt((double)partitions_no));
			futhark_entry_do_grid_dbscan_2d_f64(ctx, &dbscan_res,
		...
		mylog(logfile, "DBSCAN completed.");
	...
	// Output data via appenders
	  	...
	  	if(duckdb_query(con, "CREATE TEMP TABLE output_tbl (is_core BOOLEAN, cluster_id BIGINT);", NULL) == DuckDBError) {
