	...
	for(int64_t iter_j=0; iter_j<ITER; iter_j++) {
		...
		// Query to read data from input file.
			...
			if(!do_sampling)
				...
				sprintf(query_str,
					"SELECT (#1)::DOUBLE, (#2)::DOUBLE FROM read_csv('%s') \
		...
		// Do dbscan
			...
			struct futhark_opaque_indexed_data_2d_f64 *indexed_dat;
			futhark_entry_index_dataset_2d_f64(ctx, &indexed_dat, EPS, SUBDIV, col1, col2);
			futhark_free_f64_1d(ctx, col1);
			futhark_free_f64_1d(ctx, col2);

			struct futhark_opaque_partition_info_f64 *partInfo;
			futhark_entry_get_part_info_2d_f64(ctx, &partInfo, false, EPS, indexed_dat);

			struct futhark_opaque_partition_info_f64 *partInfo_bd;
			futhark_entry_get_part_info_2d_f64(ctx, &partInfo_bd, true, EPS, indexed_dat);
			...
			futhark_entry_get_neighbour_counts_2d_f64(
			...
			struct futhark_opaque_isolated_core_pts_2d_f64 *only_core_pts;
			futhark_entry_isolate_core_pts_2d_f64(ctx, &only_core_pts, ind_is_core, indexed_dat, partInfo);

			struct futhark_opaque_part_core_info *coreInfo;
			futhark_entry_get_part_core_info_2d_f64(ctx, &coreInfo, only_core_pts, indexed_dat);
			...
			futhark_entry_mk_clusters_2d_f64(ctx, &core_cid, SEED_COUNT, EPS, partInfo, only_core_pts, coreInfo);
			...
			futhark_entry_assign_cluster_ids_2d_f64(
				ctx, &all_ids,
				SEED_COUNT, EPS,
			...
			futhark_entry_deindex_results_2d(ctx, &dbscan_res, indexed_dat, ind_is_core, all_ids);
			...
			futhark_free_opaque_indexed_data_2d_f64(ctx, indexed_dat);
			futhark_free_opaque_isolated_core_pts_2d_f64(ctx, only_core_pts);
