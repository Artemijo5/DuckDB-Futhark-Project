...
#define default_INPUT_FILENAME  "10d.txt"
#define default_OUTPUT_FILENAME "futhark_dclust_10d.csv"
...
int main(int argc, char *argv[]) {
    ...
    // Initializations
		...
		int64_t SUBDIV6 = default_SUBDIV;
		int64_t SUBDIV7 = default_SUBDIV;
		int64_t SUBDIV8 = default_SUBDIV;
		int64_t SUBDIV9 = default_SUBDIV;
		int64_t SUBDIV10 = default_SUBDIV;
		...
		static struct option long_options[] = {
			...
			{"subdiv4",  required_argument, 0, '6'},
			{"subdiv7",  required_argument, 0, '7'},
			{"subdiv8",  required_argument, 0, '8'},
			{"subdiv9",  required_argument, 0, '9'},
			{"subdiv10", required_argument, 0, 'X'},
    	...
    	char ch;
	    while(
	    	(ch = getopt_long_only(argc,argv,"i:o:s:c:1:2:3:4:5:6:7:8:9:X:e:m:I:RL:",long_options,NULL)) != -1
	    ) {
	      switch(ch) {
	        case 'i':
	        ...
	        case '6':
	        	SUBDIV6 = atol(optarg); break;
	        case '7':
	        	SUBDIV7 = atol(optarg); break;
	         case '8':
	        	SUBDIV8 = atol(optarg); break;
	        case '9':
	        	SUBDIV9 = atol(optarg); break;
	        case 'X':
	        	SUBDIV10 = atol(optarg); break;
	        case 'e':
	...
	for(int64_t iter_j=0; iter_j<ITER; iter_j++) {
		...
		// Query to read data from input file.
			...
			if(!do_sampling)
				sprintf(query_str,
					...
					(#6)::DOUBLE,(#7)::DOUBLE,(#8)::DOUBLE,(#9)::DOUBLE,(#10)::DOUBLE \
		...
		// Init futhark arrays
			...
			struct futhark_f64_1d *col6;
			struct futhark_f64_1d *col7;
			struct futhark_f64_1d *col8;
			struct futhark_f64_1d *col9;
			struct futhark_f64_1d *colX;
			...
			futhark_entry_init_column_f64(ctx, &col6, DATASET_SIZE);
			futhark_entry_init_column_f64(ctx, &col7, DATASET_SIZE);
			futhark_entry_init_column_f64(ctx, &col8, DATASET_SIZE);
			futhark_entry_init_column_f64(ctx, &col9, DATASET_SIZE);
			futhark_entry_init_column_f64(ctx, &colX, DATASET_SIZE);
		...
		// Read duckdb chunks and copy into arrays
			...
			while(true) {
				...
				duckdb_vector vec6 = duckdb_data_chunk_get_vector(cnk,5);
				duckdb_vector vec7 = duckdb_data_chunk_get_vector(cnk,6);
				duckdb_vector vec8 = duckdb_data_chunk_get_vector(cnk,7);
				duckdb_vector vec9 = duckdb_data_chunk_get_vector(cnk,8);
				duckdb_vector vecX = duckdb_data_chunk_get_vector(cnk,9);
				...
				double *dat6 = (double*)duckdb_vector_get_data(vec6);
				double *dat7 = (double*)duckdb_vector_get_data(vec7);
				double *dat8 = (double*)duckdb_vector_get_data(vec8);
				double *dat9 = (double*)duckdb_vector_get_data(vec9);
				double *datX = (double*)duckdb_vector_get_data(vecX);
				...
				struct futhark_f64_1d *ft_dat6 = futhark_new_f64_1d(ctx, dat6, this_rows);
				struct futhark_f64_1d *ft_dat7 = futhark_new_f64_1d(ctx, dat7, this_rows);
				struct futhark_f64_1d *ft_dat8 = futhark_new_f64_1d(ctx, dat8, this_rows);
				struct futhark_f64_1d *ft_dat9 = futhark_new_f64_1d(ctx, dat9, this_rows);
				struct futhark_f64_1d *ft_datX = futhark_new_f64_1d(ctx, datX, this_rows);
				...
				struct futhark_f64_1d *col6_tmp;
				struct futhark_f64_1d *col7_tmp;
				struct futhark_f64_1d *col8_tmp;
				struct futhark_f64_1d *col9_tmp;
				struct futhark_f64_1d *colX_tmp;
				...
				futhark_entry_write_column_f64(ctx, &col6_tmp, num_rows, ft_dat6, col6);
				futhark_entry_write_column_f64(ctx, &col7_tmp, num_rows, ft_dat7, col7);
				futhark_entry_write_column_f64(ctx, &col8_tmp, num_rows, ft_dat8, col8);
				futhark_entry_write_column_f64(ctx, &col9_tmp, num_rows, ft_dat9, col9);
				futhark_entry_write_column_f64(ctx, &colX_tmp, num_rows, ft_datX, colX);
				...
				futhark_free_f64_1d(ctx, col6);
				futhark_free_f64_1d(ctx, col7);
				futhark_free_f64_1d(ctx, col8);
				futhark_free_f64_1d(ctx, col9);
				futhark_free_f64_1d(ctx, colX);
		...
		// Crop arrays if necessary
			...
			if(num_rows < DATASET_SIZE) {
				...
				futhark_entry_crop_column_f64(ctx, &col6_tmp, 0, num_rows, col6);
				futhark_entry_crop_column_f64(ctx, &col7_tmp, 0, num_rows, col7);
				futhark_entry_crop_column_f64(ctx, &col8_tmp, 0, num_rows, col8);
				futhark_entry_crop_column_f64(ctx, &col9_tmp, 0, num_rows, col9);
				futhark_entry_crop_column_f64(ctx, &colX_tmp, 0, num_rows, colX);
		...
		// Do dbscan
			...
			struct futhark_opaque_indexed_data_10d_f64 *indexed_dat;
			futhark_entry_index_dataset_10d_f64(
				ctx, &indexed_dat, EPS,
			...
			futhark_entry_get_part_info_10d_f64(ctx, &partInfo, false, EPS, indexed_dat);

			struct futhark_opaque_partition_info_f64 *partInfo_bd;
			futhark_entry_get_part_info_10d_f64(ctx, &partInfo_bd, true, EPS, indexed_dat);
			...
			// Record true subdiv and #cells.
				...
				futhark_project_opaque_indexed_data_10d_f64_subdiv(ctx, &ft_subdiv, indexed_dat);

				struct futhark_i64_1d *ft_cell_ids;
				futhark_project_opaque_indexed_data_10d_f64_cell_ids(ctx, &ft_cell_ids, indexed_dat);
				...
				printf(
					...
					true_subdiv[5], true_subdiv[6], true_subdiv[7], true_subdiv[8], true_subdiv[9]
				);
			...
			futhark_entry_get_neighbour_counts_10d_f64(
			...
			struct futhark_opaque_isolated_core_pts_10d_f64 *only_core_pts;
			futhark_entry_isolate_core_pts_10d_f64(ctx, &only_core_pts, ind_is_core, indexed_dat, partInfo);

			struct futhark_opaque_part_core_info *coreInfo;
			futhark_entry_get_part_core_info_10d_f64(ctx, &coreInfo, only_core_pts, indexed_dat);
			...
			futhark_entry_mk_clusters_10d_f64(ctx, &core_cid, SEED_COUNT, EPS, partInfo, only_core_pts, coreInfo);
			...
			futhark_entry_assign_cluster_ids_10d_f64(
				ctx, &all_ids,
				SEED_COUNT, EPS,
			...
			futhark_entry_deindex_results_10d(ctx, &dbscan_res, indexed_dat, ind_is_core, all_ids);
			...
			futhark_free_opaque_indexed_data_10d_f64(ctx, indexed_dat);
			futhark_free_opaque_isolated_core_pts_10d_f64(ctx, only_core_pts);
