...
#define default_INPUT_FILENAME  "7d.txt"
#define default_OUTPUT_FILENAME "futhark_dclust_7d.csv"
...
int main(int argc, char *argv[]) {
    ...
    // Initializations
    	...
    	char ch;
	    while(
	    	(ch = getopt_long_only(argc,argv,"i:o:s:c:1:2:3:4:5:6:7:e:m:I:RL:",long_options,NULL)) != -1
	    ) {
	      switch(ch) {
	        case 'i':
	...
	for(int64_t iter_j=0; iter_j<ITER; iter_j++) {
		...
		// Query to read data from input file.
			...
			if(!do_sampling)
				sprintf(query_str,
					"SELECT \
					(#1)::DOUBLE,(#2)::DOUBLE,(#3)::DOUBLE,(#4)::DOUBLE,(#5)::DOUBLE,(#6)::DOUBLE,(#7)::DOUBLE \
		...
		// Do dbscan
			...
			struct futhark_opaque_indexed_data_7d_f64 *indexed_dat;
			futhark_entry_index_dataset_7d_f64(
				ctx, &indexed_dat, EPS,
				SUBDIV1, SUBDIV2, SUBDIV3, SUBDIV4, SUBDIV5, SUBDIV6, SUBDIV7,
			...
			futhark_entry_get_part_info_7d_f64(ctx, &partInfo, false, EPS, indexed_dat);

			struct futhark_opaque_partition_info_f64 *partInfo_bd;
			futhark_entry_get_part_info_7d_f64(ctx, &partInfo_bd, true, EPS, indexed_dat);
			...
			// Record true subdiv and #cells.
				...
				futhark_project_opaque_indexed_data_7d_f64_subdiv(ctx, &ft_subdiv, indexed_dat);

				struct futhark_i64_1d *ft_cell_ids;
				futhark_project_opaque_indexed_data_7d_f64_cell_ids(ctx, &ft_cell_ids, indexed_dat);
			...
			futhark_entry_get_neighbour_counts_7d_f64(
			...
			struct futhark_opaque_isolated_core_pts_7d_f64 *only_core_pts;
			futhark_entry_isolate_core_pts_7d_f64(ctx, &only_core_pts, ind_is_core, indexed_dat, partInfo);

			struct futhark_opaque_part_core_info *coreInfo;
			futhark_entry_get_part_core_info_7d_f64(ctx, &coreInfo, only_core_pts, indexed_dat);
			...
			futhark_entry_mk_clusters_7d_f64(ctx, &core_cid, SEED_COUNT, EPS, partInfo, only_core_pts, coreInfo);
			...
			futhark_entry_assign_cluster_ids_7d_f64(
				ctx, &all_ids,
				SEED_COUNT, EPS,
			...
			futhark_entry_deindex_results_7d(ctx, &dbscan_res, indexed_dat, ind_is_core, all_ids);
			...
			futhark_free_opaque_indexed_data_7d_f64(ctx, indexed_dat);
			futhark_free_opaque_isolated_core_pts_7d_f64(ctx, only_core_pts);
