...
int main(int argc, char *argv[]) {
	...
	// Fetch tables into memory.
	  	...
	  	sprintf(fetch_R,"CREATE TEMP TABLE %s_tmp AS (FROM %s);", R_name, R_name);
	  	...
	  	sprintf(fetch_S,"CREATE TEMP TABLE %s_tmp AS (FROM %s);", S_name, S_name);
	...
	// Sort tables.
	  	...
	  	sprintf(sort_R,"CREATE TEMP TABLE %s_sorted AS (FROM %s_tmp ORDER BY %s);", R_name, R_name, k_name);
	  	...
	  	sprintf(sort_S,"CREATE TEMP TABLE %s_sorted AS (FROM %s_tmp ORDER BY %s);", S_name, S_name, k_name);
