...
#define default_MAGNITUDE_INNER (float)1.618
#define default_CORR_FACTOR (float)0.7
#define default_EXP (float)2.0
...
int main(int argc, char *argv[]) {
	// Parse command line arguments
		// Initializations
			...
			float MAGNITUDE_INNER = default_MAGNITUDE_INNER;
			float CORR_FACTOR = default_CORR_FACTOR;
			float EXP = default_EXP;
		...
		static struct option long_options[] =
		  {
		      ...
		      {"magnitude_inner", required_argument, 0, 'N'},
		      {"corr_factor", required_argument, 0, 'C'},
		      {"exponent", required_argument, 0, 'S'},
		...
		char ch;
		while(
		  (ch = getopt_long_only(argc,argv,"L:B:t:T:D:M:N:C:S:f:m:d:",long_options,NULL)) != -1
		) {
		  switch(ch) {
		    case 'L':
		    ...
		    case 'N':
		    	MAGNITUDE_INNER = (float)atof(optarg); break;
		    case 'C':
		    	CORR_FACTOR = (float)atof(optarg); break;
		    case 'S':
		    	EXP = (float)atof(optarg); break;
		    case 'f':
	...
	// Initialise logger
    FILE* logfile = loginit(LOGFILE, "mk_crescent : Generating test data.");
    if(LOGFILE && !logfile) {
	...
	// Create initial table (dim random samples)
		char queryStr[250 + strlen(TABLE_NAME) + 20*(DIM)];
	    int queryLen = sprintf(queryStr, "CREATE OR REPLACE TEMP TABLE tmp_mk_crescent_buffer (");
	    ...
	    char insertQueryStr[250 + 100*(DIM)];
	    int insertQueryLen = sprintf(insertQueryStr, "INSERT INTO tmp_mk_crescent_buffer (SELECT ");
	    for(idx_t i=0; i<DIM; i++) {
	...
	// Transform and store data
	    duckdb_result res;
	    duckdb_query(con, "SELECT * FROM tmp_mk_crescent_buffer;", &res);
	    bool isTblExhausted = false;
	    mylog(logfile, "Processing buffer data...");
	    while(!isTblExhausted) {
	    	// Allocate buffers
		    	void **buffers  = malloc((DIM)*sizeof(float*));
		    	for(int64_t d=0; d<DIM; d++) {
		    		buffers[d] = malloc(BUFFER_SIZE*sizeof(float));
		    	}
		    	void *flatbuff = malloc((DIM)*BUFFER_SIZE*sizeof(float));
	    	...
	    	// Process data through futhark
		    	struct futhark_f32_2d *buff_in_ft = futhark_new_f32_2d(ctx, flatbuff, DIM, row_count);

		    	struct futhark_f32_2d *buff_out_ft;
		    	futhark_entry_hyperbolic_float(ctx, &buff_out_ft, CORR_FACTOR, MAGNITUDE_INNER, MAGNITUDE, buff_in_ft, EXP);
